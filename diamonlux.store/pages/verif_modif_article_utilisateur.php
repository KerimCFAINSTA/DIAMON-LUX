<?php
    ...
    if(!empty($_GET['id'])){
        ...
        if(!empty($_POST['nom'])){
            $q = 'UPDATE article SET nom = :nom WHERE id=:id'; 
            $req = $bdd-> prepare($q); 
            $req->execute([
                'nom' => $_POST['nom'], 
                'id' => $id
        ...
        if(!empty($_POST['marque'])){
            $q = 'UPDATE article SET marque = :marque WHERE id=:id'; 
            $req = $bdd-> prepare($q); 
            $req->execute([
                'marque' => $_POST['marque'], 
                'id' => $id
        ...
        if(!empty($_POST['prix'])){
            if($_POST['prix']>0){
                $q = 'UPDATE article SET prix = :prix WHERE id=:id'; 
                $req = $bdd-> prepare($q); 
                $req->execute([
                    'prix' => $_POST['prix'], 
                    'id' => $id
        ...
        if(!empty($_POST['secteur'])){
            $q = 'UPDATE article SET secteur = :secteur WHERE id=:id'; 
            $req = $bdd-> prepare($q); 
            $req->execute([
        ...
        if (!empty($_FILES['image']) && $_FILES['image']['error'] != 4) {
            ...
            $q = 'UPDATE article SET image = :image WHERE id = :id';
            $req = $bdd->prepare($q); 
            $reponse = $req->execute([
                'image' => $filename,
                'id' => $id
        ...
        header('location: article_utilisateur.php?message=' . $msg);
        exit;
